...
class UNet_original(torch.nn.Module):
...
class UNet_small(torch.nn.Module):
    """Implementation of the U-Net architecture
    "U-Net: Convolutional Networks for Biomedical Image Segmentation"
    by Olaf Ronneberger, Philipp Fischer, and Thomas Brox (2015)
    https://arxiv.org/pdf/1505.04597.pdf

    :: here used for time series segmentation
    :: n_input_variables
    """


    def __init__(self, n_classes, n_bands, batch_norm=True):
        """
        """
        self.name = 'UNet_small'
        self.n_classes = n_classes
    ...
    def contract(self):
        ...
        blocks = []
        old = self.n_bands
        for i, size in enumerate(self.filter_sizes): ## building blocks according to filter sizes

            mpool = nn.MaxPool1d(kernel_size=2)
            conv1 = nn.Conv1d(old, size, kernel_size=3)
            conv2 = nn.Conv1d((size ), size, kernel_size=3)
            relu = nn.ReLU(True)

            convs = [mpool, conv1, relu, conv2, relu]

            if self.batch_norm:
                b_norm = nn.LazyBatchNorm1d(size)
                convs = [mpool, conv1, b_norm, relu, conv2, b_norm, relu]
            if i == 0:
                convs = convs[1:]
            block = nn.Sequential(*convs)
...
if __name__ == "__main__":
    # A full forward pass
    im = torch.randn( 2, 4, 32)

    ## n classes ; n_bands
    model = UNet_small(5,4)
